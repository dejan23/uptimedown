  ...
  try {
    ...
    if (checks && checks.length > 0) {
      checks.map((originalCheckData) => {
        if (Date.now() - originalCheckData.lastChecked >= originalCheckData.interval * 1000 * 60) {
...
workers.validateCheckData = function (originalCheckData) {
  ...
  originalCheckData.timeoutSeconds = typeof (originalCheckData.timeoutSeconds) == 'number' && originalCheckData.timeoutSeconds % 1 === 0 && originalCheckData.timeoutSeconds >= 1 && originalCheckData.timeoutSeconds <= 30 ? originalCheckData.timeoutSeconds : false;
  originalCheckData.interval = typeof (originalCheckData.interval) == 'number' && originalCheckData.interval % 1 === 0 && originalCheckData.interval >= 1 && originalCheckData.interval <= 5 ? originalCheckData.interval : false;

  // Set the keys that may not be set (if the workers have never seen this check before)
  originalCheckData.state = typeof (originalCheckData.state) == 'string' && ['up', 'down', 'wait...'].indexOf(originalCheckData.state) > -1 ? originalCheckData.state : 'down';
...
workers.performCheck = async (originalCheckData) => {
  ...
  const req = request(requestDetails, function (error, res) {
    ...
    let status = '';
    let responseTime = '';
    let responseTimeInt = false; //i use this for log / avrage response because it needs to be an integer
    ...
    if (!outcomeSent) {
      workers.processCheckOutcome(originalCheckData, checkOutcome, responseTimeInt);
      outcomeSent = true;
    }
  })
...
workers.processCheckOutcome = async (originalCheckData, checkOutcome, responseTimeInt) => {
  const currentEvent = await Events.findOne({ 'checkId': originalCheckData._id }).sort({ 'createdAt': -1 });
  ...
  if (currentEvent.state !== 'started') {
    ...
    console.log('alert', originalCheckData.lastChecked && originalCheckData.state !== state ? true : false)
    alertWarranted = originalCheckData.lastChecked && originalCheckData.state !== state ? true : false;
  }
  ...
  if (state === 'up' && currentEvent.state === 'started' ||
  ...
  originalCheckData.successCodes.indexOf(checkOutcome.responseCode) > -1 ? null : response = 'wrong response';
  if (!!checkOutcome.error) {
    if (checkOutcome.error.value.code === 'ESOCKETTIMEDOUT')
      response = 'Connection Timeout'
    if (checkOutcome.error.value.code === 'ENOTFOUND')
      response = 'Not Found'
  ...
  if (state === 'down' && currentEvent.state === 'started' ||
...
workers.alertUserToStatusChange = async (newCheckData, response) => {
  ...
  const msgWReason = 'Alert: Your check for ' + newCheckData.method.toUpperCase() + ' ' + newCheckData.protocol + '://' + newCheckData.url + ' is currently ' + newCheckData.state + '. Reason - "' + response + '" <br /> Uptimedown <br /> Web: https://uptimedown.net <br /> Email: dejan.dvte@gmail.com'
  const msgWOReason = 'Alert: Your check for ' + newCheckData.method.toUpperCase() + ' ' + newCheckData.protocol + '://' + newCheckData.url + ' is currently ' + newCheckData.state + '. <br /> Uptimedown <br /> Web: https://uptimedown.net <br /> Email: dejan.dvte@gmail.com'
